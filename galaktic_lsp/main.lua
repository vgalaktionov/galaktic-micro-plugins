...
function init()
    config.RegisterCommonOption("galakticlsp", "statusline", true)
    config.RegisterCommonOption("galakticlsp", "compactfilename", true)
    ...
    config.AddRuntimeFile("galaktic_lsp", config.RTHelp, "help/galaktic-lsp.md")
    if config.GetGlobalOption("galakticlsp.statusline") then
        local ok, current = pcall(config.GetGlobalOption, "statusformatl")
        if ok and type(current) == "string" then
            local nextFormat = current
            if config.GetGlobalOption("galakticlsp.compactfilename") then
                if string.find(nextFormat, "%$%((filename)%)") then
                    nextFormat = string.gsub(nextFormat, "%$%((filename)%)", "$(galaktic_lsp.filename)")
                elseif not string.find(nextFormat, "galaktic_lsp.filename", 1, true) then
                    nextFormat = "$(galaktic_lsp.filename) " .. nextFormat
                end
            end
            if not string.find(nextFormat, "galaktic_lsp.status", 1, true) then
                local suffix = " | lsp:$(galaktic_lsp.status)"
                nextFormat = nextFormat .. suffix
            end
            if nextFormat ~= current then
                pcall(config.SetGlobalOption, "statusformatl", nextFormat)
            end
        end
    end
    if settings.defaultBindings ~= nil then
        local overwrite = settings.defaultBindingsOverwrite == true
        for key, action in pairs(settings.defaultBindings) do
            pcall(config.TryBindKey, key, action, overwrite)
        end
    end
    micro.SetStatusInfoFn("galaktic_lsp.status")
    micro.SetStatusInfoFn("galaktic_lsp.filename")
...
function status(buf)
    ...
    if buf then
        ...
        for _, client in pairs(activeConnections) do
            if client:supportsFiletype(filetype) then
...
function filename(buf)
    local name = ""
    if buf and buf.Path ~= "" then
        name = go_filepath.Base(buf.Path)
    elseif buf and buf.AbsPath ~= "" then
        name = go_filepath.Base(buf.AbsPath)
    elseif buf then
        name = go_filepath.Base(buf:GetName())
    end
    if name == "" then
        return "[No Name]"
    end
...
function showLog(_, args)
    ...
    local title = string.format("[galaktic_lsp] Log for '%s' (%s)", foundClient.name, foundClient.clientId)
...
function log(...)
    micro.Log("[galaktic_lsp]", unpack(arg))
end

function display_error(...)
    micro.InfoBar():Error("[galaktic_lsp] ", unpack(arg))
end

function display_info(...)
    micro.InfoBar():Message("[galaktic_lsp] ", unpack(arg))
end
    ...
    client:request(req, {
        ...
        onResult = function(result)
            local showHoverInfo = function(data)
                local bf = buffer.NewBuffer(data, "[galaktic_lsp] hover")
...
local function formatDocumentOnSave(bufpane, onDone)
    local filetype = bufpane.Buf:FileType()
    local client = findClientSilent(filetype, "documentFormattingProvider")
    if not client then
        onDone(false)
        return
    end
    ...
    client:request(req, {
        onResult = function(result)
            if result ~= json.null and next(result) ~= nil then
                applyTextEdits(bufpane.Buf, result)
            end
            onDone(true)
        end,
        onError = function(err)
            defaultOnErrorHandler(method)(err)
            onDone(false)
        end
    })
end
...
function renameAction(bufpane, args)
    ...
    if #args > 0 then -- `lsp rename newName`
        ...
        micro.InfoBar():Prompt(
            string.format("[galaktic_lsp] rename symbol at line %d column %d to: ", cursor.Y + 1, cursor.X + 1),
            "", -- placeholder
            "galaktic_lsp-rename-symbol", -- prompt type (prompts with same type share history)
            nil, -- event callback
...
function findReferencesAction(bufpane)
    ...
    client:request(req, {
        ...
        onResult = function(result)
            ...
            showReferenceLocations("[galaktic_lsp] references", result)
        end,
        onError = defaultOnErrorHandler(method)
    })
end
...
function documentSymbolsAction(bufpane)
    ...
    client:request(req, {
        ...
        onResult = function(result)
            ...
            showSymbolLocations("[galaktic_lsp] document symbols", symbolLocations, symbolLabels)
        end,
        onError = defaultOnErrorHandler(method)
    })
end
        ...
        if file then
            ...
            for idx, diagnostic in pairs(diagnostics) do
                ...
                if cursor.Loc.Y == startLoc.Y then
                    ...
                    local bufTitle = string.format("[galaktic_lsp] %s diagnostics #%d", client.clientId, idx)
...
function onBufferOpen(buf)
    ...
    if string.startsWith(buf:GetName(), "[galaktic_lsp]") then return end
...
function onSave(bufpane)
    local filePath = bufpane.Buf.AbsPath
    if settings.formatOnSave then
        if format_on_save_inflight[filePath] then
            format_on_save_inflight[filePath] = nil
            for _, client in pairs(activeConnections) do
        ...
        formatDocumentOnSave(bufpane, function(didFormat)
            if didFormat then
                format_on_save_inflight[filePath] = true
                bufpane:Save()
            else
                for _, client in pairs(activeConnections) do
                    client:didSave(bufpane.Buf)
...
function showDiagnostics(buf, owner, diagnostics)
    ...
    for _, diagnostic in pairs(diagnostics) do
        ...
        if settings.showDiagnostics[severity] then
            ...
            msg = string.format("[galaktic_lsp] %s%s", extraInfo or "", msg)
...
function findClientSilent(filetype, capability)
    if next(activeConnections) == nil then
        return nil
    end
